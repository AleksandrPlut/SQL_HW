...
# Задание 2. Вывести магазин в котор продается кот Мурзик (выполнить двумя способами)
...
# 2.2
SELECT s.shopname, Murz.name
FROM shops s
JOIN
(SELECT name, shops_id 
FROM cats
WHERE cats.name = "Murzik") AS Murz
WHERE s.id = Murz.shops_id;


# Задание 3. Вывести магазины, в которых не продаются коты "Мурзик" и "Зуза"

SELECT s.shopname, shops2.name
FROM shops s
JOIN
(SELECT name, shops_id 
FROM cats
WHERE cats.name != "Murzik" AND cats.name != "Zuza") AS shops2
WHERE s.id = shops2.shops_id;
...
SELECT a.an_name "Название анализа", 
a.an_cost "Себестоимость анализи", 
a.an_price "Розничная цена анализа", 
o.ord_datetime "Дата и время заказа"
FROM Analysis a
JOIN Orders o
ON a.an_id = o.ord_an
WHERE o.ord_datetime BETWEEN "2020-02-05" AND "2020-02-13"
ORDER BY o.ord_datetime;    # Специально отсортировал по дате, чтоб легче было проверить все ли анализы попали в выборку
